...
try:
    ...
    # Fallback for direct execution if needed, implies sys.path hack works
    from mt5_bridge import MT5DataClient 
...
def draw_price_chart(symbol: str = "XAUUSD") -> Optional[str]:
    ...
    try:
        ...
        if not client.connect():
            logger.error("❌ Lỗi kết nối MT5.")
            return None
            
        # Lấy 80 nến (Số lượng vừa đủ để nến trông cân đối trên khung hình rộng)
        ...
        s = mpf.make_mpf_style(
            ...
            rc={
                ...
                'axes.edgecolor': '#505050'  # Trắng xám không quá nổi (Subtle Gray)
            }
        )
