...
class KeyManager:
    """Quản lý multiple OpenAI API Keys cho rotation"""
    def __init__(self, keys: List[str]):
        self.keys = keys
        self.current_index = 0
        logger.info(f"🔑 Loaded {len(keys)} OpenAI API Keys for rotation.")
    ...
    def switch_key(self) -> str:
        ...
        if len(self.keys) <= 1:
            logger.warning("⚠️ Only 1 OpenAI key available, cannot rotate.")
            return self.get_current_key()
        
        self.current_index = (self.current_index + 1) % len(self.keys)
        new_key = self.get_current_key()
        logger.info(f"🔄 Switching OpenAI API Key -> ...{new_key[-8:]}")
        return new_key

class OpenAIService(AIService):
    def __init__(self):
        # Hỗ trợ multi-key (comma-separated)
        openai_key_str = config.OPENAI_API_KEY or ""
        api_keys = [k.strip() for k in openai_key_str.split(',') if k.strip()]
    ...
    async def generate_content(self, prompt: str, schema: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        while attempt < retries:
            try:
                ...
                if "429" in error_msg or "rate" in error_msg or "quota" in error_msg:
                    logger.warning(f"⚠️ OpenAI Rate Limit / Quota Error: {e}")
        ...
        logger.error("❌ OpenAI Hết lượt thử (Retries Exhausted).")
        return None
